...
export class ClassSymbolExtractor extends BaseSymbolExtractor {
     ...
     */
    public extractClass(path: NodePath<t.ClassDeclaration>, filePath: string): CodeSymbol | null {
     ...
     */
    public extractClassMembers(path: NodePath<t.ClassDeclaration>, filePath: string): CodeSymbol[] {
        const node = path.node;
    ...
    public extract(path: NodePath<t.ClassDeclaration>, filePath: string): CodeSymbol[] {
        const classSymbol = this.extractClass(path, filePath);
        if (!classSymbol) {
            return [];
        }

        const members = this.extractClassMembers(path, filePath);
