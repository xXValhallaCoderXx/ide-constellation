...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const healthReportDisposable = vscode.commands.registerCommand(
    "constellation.healthReport",
    async () => {
      ...
      await vscode.window.withProgress(
        ...
        async (progress) => {
          try {
            ...
            progress.report({
              increment: 20,
              message: "Initializing health analyzer...",
            });

            // Import services dynamically to avoid circular dependencies
            const { HealthAnalyzer } = await import(
              "./services/health-analyzer.service"
            );
            ...
            if (!graph) {
              progress.report({
                increment: 0,
                message: "No graph data found. Scanning project first...",
              });
              ...
              if (!graph) {
                throw new Error(
                  'Unable to load graph data. Please run "Constellation: Scan Project" first.'
                );
              }
            }
            ...
            progress.report({
              increment: 20,
              message: "Analyzing file complexity and churn...",
            });
            ...
            progress.report({
              increment: 30,
              message: "Opening health dashboard...",
            });
            ...
            vscode.window.showErrorMessage(
              `Health report failed: ${errorMessage}`
            );
          }
        }
      );
  ...
  const healthReportGraphDisposable = vscode.commands.registerCommand(
    "constellation.healthReportGraph",
    async () => {
      ...
      await vscode.window.withProgress(
        ...
        async (progress) => {
          try {
            ...
            progress.report({
              increment: 30,
              message: "Applying heatmap to graph...",
            });

            // Apply heatmap directly to graph
            if (webviewManager) {
              await webviewManager.navigateFromDashboardToGraph(
                analysis.riskScores
              );
            }
            ...
            vscode.window.showErrorMessage(
              `Health report graph failed: ${errorMessage}`
            );
          }
        }
      );
  ...
  const clearHeatmapDisposable = vscode.commands.registerCommand(
    "constellation.clearHeatmap",
    () => {
      ...
      try {
        if (webviewManager) {
          ...
          if (panel) {
            ...
            vscode.window.showInformationMessage(
              "Health heatmap cleared from graph"
            );
          } else {
            vscode.window.showWarningMessage(
              "No graph panel is currently open"
            );
          }
        }
      } catch (error) {
      ...
      await vscode.window.withProgress(
        ...
        async (progress) => {
          try {
            ...
            const { HealthDisplayService } = await import(
              "./services/health-display.service"
            );
            ...
            const summaryMessage =
              HealthDisplayService.createSummaryMessage(analysis);
            vscode.window.showInformationMessage(summaryMessage);
          } catch (error) {
            const errorMessage =
              error instanceof Error ? error.message : String(error);
            ...
            vscode.window.showErrorMessage(
              `Health analysis failed: ${errorMessage}`
            );
          }
        }
      );
  ...
  const debugLaunchDisposable = vscode.commands.registerCommand(
    "kiro-constellation.debugLaunchMcp",
    async () => {
      if (!IS_DEV) {
      ...
      try {
        ...
        log(
          `[DEBUG] Spawning MCP server with: ${process.execPath} ${serverScriptPath}`
        );
        ...
        const init = {
          ...
          params: {
            ...
            capabilities: {},
            clientInfo: { name: "kiro-constellation", version: "dev" },
          },
        };
        const list = {
          jsonrpc: "2.0",
        ...
        const call = {
          ...
          params: {
            ...
            arguments: { message: "Hello from debug command" },
          },
        };
        const ping = {
          jsonrpc: "2.0",
          id: 4,
          method: "tools/call",
          params: { name: "constellation_ping", arguments: {} },
        };
        ...
        log(
          `[DEBUG] Failed to launch MCP server: ${
